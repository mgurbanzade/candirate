import cx from 'classnames';
import { useModal } from '@hooks/useModal';
import { Question } from '@gql/types/graphql';
import { useMutation } from '@apollo/client';
import { UIQuestionType } from '@lib/ui-types';
import { REMOVE_QUESTIONS } from '@gql/mutations/questions';

type Props = {
  setQuestions: any;
  isSelectState: boolean;
  selectedQuestionIds: number[];
  setIsSelectState: any;
};

const QuestionsListHeader = ({
  setQuestions,
  isSelectState,
  setIsSelectState,
  selectedQuestionIds,
}: Props) => {
  const [removeQuestions] = useMutation(REMOVE_QUESTIONS);
  const { setIsVisible } = useModal();
  const handleNewQuestion = () => {
    setQuestions((prevQuestions: Question[]) => [
      {
        id: Date.now(),
        isNew: true,
        title: "Edit new question's title",
        points: 5,
      } as UIQuestionType,
      ...prevQuestions,
    ]);
  };

  const handleDeleteQuestions = async () => {
    try {
      const { errors } = await removeQuestions({
        variables: {
          ids: selectedQuestionIds,
        },
      });

      if (!errors) {
        setQuestions((prevQuestions: Question[]) =>
          prevQuestions.filter(
            (question) => !selectedQuestionIds.includes(question.id as number),
          ),
        );
        setIsSelectState(false);
      }
    } catch (e) {
      console.log(e);
    }
  };

  return (
    <div className="border-b border-gray-200 bg-white px-4 py-5 sm:px-6">
      <div className="-ml-4 -mt-2 flex flex-wrap items-center justify-between sm:flex-nowrap">
        <div className="ml-4 mt-2">
          <h3 className="text-lg font-medium leading-6 text-gray-900">
            Questions
          </h3>
        </div>
        <div className="ml-4 mt-2 flex items-center flex-shrink-0">
          {isSelectState && (
            <button
              type="button"
              onClick={handleDeleteQuestions}
              disabled={selectedQuestionIds.length === 0}
              className={cx(
                'inline-flex items-center rounded-md bg-red-500 hover:bg-red-700 px-4 py-2 text-sm font-medium text-white',
                {
                  'opacity-70': selectedQuestionIds.length === 0,
                },
              )}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="currentColor"
                className="w-6 h-6"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0"
                />
              </svg>
            </button>
          )}
          {isSelectState && (
            <button
              type="button"
              onClick={() => setIsVisible(true)}
              disabled={selectedQuestionIds.length === 0}
              className={cx(
                'relative inline-flex items-center rounded-md border border-transparent bg-yellow-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-yellow-700 ml-3',
                {
                  'opacity-70': selectedQuestionIds.length === 0,
                },
              )}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="currentColor"
                className="w-6 h-6"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5m-9-6h.008v.008H12v-.008zM12 15h.008v.008H12V15zm0 2.25h.008v.008H12v-.008zM9.75 15h.008v.008H9.75V15zm0 2.25h.008v.008H9.75v-.008zM7.5 15h.008v.008H7.5V15zm0 2.25h.008v.008H7.5v-.008zm6.75-4.5h.008v.008h-.008v-.008zm0 2.25h.008v.008h-.008V15zm0 2.25h.008v.008h-.008v-.008zm2.25-4.5h.008v.008H16.5v-.008zm0 2.25h.008v.008H16.5V15z"
                />
              </svg>
            </button>
          )}
          <button
            type="button"
            onClick={() => setIsSelectState(!isSelectState)}
            className={cx(
              'inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 ml-3',
              {
                'bg-gray-300 hover:bg-gray-300': isSelectState,
              },
            )}
          >
            Select
          </button>
          <button
            type="button"
            onClick={handleNewQuestion}
            disabled={isSelectState}
            className="relative inline-flex items-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 ml-3"
          >
            New Question
          </button>
        </div>
      </div>
    </div>
  );
};

export default QuestionsListHeader;
